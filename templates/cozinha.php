...
try {
    ...
    $stmt = $conn->query("SELECT p.id, p.mesa_numero, p.valor, p.status_pagamento, p.status_preparo, p.tipo_entrega, p.observacao, p.cliente_nome, p.cep_entrega, p.endereco_entrega, p.numero_endereco, p.complemento_endereco, p.bairro_endereco, p.cidade_endereco, p.uf_endereco, p.criado_em,
            ...
            <?php else: ?>
                <div class="tabela-wrap">
                    <table class="tabela tabela-cozinha">
                        <thead>
                            <tr><th>Pedido</th><th>Tipo</th><th>Mesa / Retirada</th><th>Itens</th><th>Observação</th><th>Recebido às</th><th>Pagamento</th><th>Preparo</th><th>Ação</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach ($pedidos as $pedido): ?>
                                <?php $estaPronto = $pedido['status_preparo'] === Pedidos::PRONTO; $tipo = Pedidos::textoTipoEntrega((string) ($pedido['tipo_entrega'] ?? Pedidos::TIPO_MESA)); ?>
                                <tr>
                                    <td>#<?php echo (int) $pedido['id']; ?></td>
                                    <td><?php echo htmlspecialchars($tipo); ?></td>
                                    <td class="mesa-cozinha"><?php echo (($pedido['tipo_entrega'] ?? Pedidos::TIPO_MESA) === Pedidos::TIPO_MESA) ? 'Mesa ' . (int) $pedido['mesa_numero'] : (($pedido['tipo_entrega'] ?? '') === Pedidos::TIPO_VIAGEM ? 'Retirada' : 'Entrega'); ?></td>
